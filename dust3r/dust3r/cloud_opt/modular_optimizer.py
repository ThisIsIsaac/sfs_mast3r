...
class ModularPointCloudOptimizer (BasePCOptimizer):
    ...
    def depth_to_pts3d(self):
        ...
        rel_ptmaps = [geometry.depthmap_to_pts3d(depth[i][None], focal_ex(i), pp=pp[i:i+1])[0] for i in range(im_poses.shape[0])]
        # project to world frame
        return [geometry.geotrf(pose, ptmap) for pose, ptmap in zip(im_poses, rel_ptmaps)]
