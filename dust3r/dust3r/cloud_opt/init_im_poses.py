...
def init_from_pts3d(self, pts3d, im_focals, im_poses):
    ...
    if nkp == 1:
        ...
        for img_pts3d in pts3d:
            img_pts3d[:] = geometry.geotrf(trf, img_pts3d)
    ...
    if self.has_im_poses:
        for i in range(self.n_imgs):
            ...
            depth = geometry.geotrf(geometry.inv(cam2world), pts3d[i])[..., 2]
...
def minimum_spanning_tree(imshapes, edges, pred_i, pred_j, conf_i, conf_j, im_conf, min_conf_thr,
    ...
    while todo:
        ...
        if i in done:
            ...
            pts3d[j] = geometry.geotrf(trf, pred_j[i_j])
            done.add(j)
            ...
            trf = sRT_to_4x4(s, R, T, device)
            pts3d[i] = geometry.geotrf(trf, pred_i[i_j])
            done.add(i)
            msp_edges.append((i, j))
...
def fast_pnp(pts3d, focal, msk, device, pp=None, niter_PnP=10):
    ...
    return best_focal, geometry.inv(sRT_to_4x4(1, R, T, device))  # cam to world
...
def align_multiple_poses(src_poses, target_poses):
    ...
    def center_and_z(poses):
        eps = geometry.get_med_dist_between_poses(poses) / 100
