...
class BasePCOptimizer (nn.Module):
    ...
    def clean_pointcloud(self, **kw):
        cams = geometry.inv(self.get_im_poses())
    ...
    def forward(self, ret_details=False):
        ...
        for e, (i, j) in enumerate(self.edges):
            ...
            aligned_pred_i = geometry.geotrf(pw_poses[e], pw_adapt[e] * self.pred_i[i_j])
            aligned_pred_j = geometry.geotrf(pw_poses[e], pw_adapt[e] * self.pred_j[i_j])
    ...
    def show(self, show_pw_cams=False, show_pw_pts3d=False, cam_size=None, **kw):
        ...
        if show_pw_cams:
            ...
            if show_pw_pts3d:
                pts = [geometry.geotrf(pw_poses[e], self.pred_i[edge_str(i, j)]) for e, (i, j) in enumerate(self.edges)]
...
def clean_pointcloud( im_confs, K, cams, depthmaps, all_pts3d, 
    ...
    for i, pts3d in enumerate(all_pts3d):
        for j in range(len(all_pts3d)):
            ...
            proj = geometry.geotrf(cams[j], pts3d)
            proj_depth = proj[:,:,2]
            u,v = geometry.geotrf(K[j], proj, norm=1, ncol=2).round().long().unbind(-1)
