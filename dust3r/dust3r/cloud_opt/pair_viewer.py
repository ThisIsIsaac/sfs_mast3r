...
class PairViewer (BasePCOptimizer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in range(self.n_imgs):
            ...
            try:
                ...
                pose = geometry.inv(np.r_[np.c_[R, T], [(0, 0, 0, 1)]])  # cam to world
            except:
                pose = np.eye(4)
        ...
        if confs[0] > confs[1]:
            ...
            self.depth = [self.pred_i['0_1'][..., 2], geometry.geotrf(geometry.inv(rel_poses[1]), self.pred_j['0_1'])[..., 2]]
        else:
            # ptcloud is expressed in camera2
            self.im_poses = [rel_poses[0], torch.eye(4)]  # I, cam1-to-cam2
            self.depth = [geometry.geotrf(geometry.inv(rel_poses[0]), self.pred_j['1_0'])[..., 2], self.pred_i['1_0'][..., 2]]
    ...
    def depth_to_pts3d(self):
        ...
        for d, intrinsics, im_pose in zip(self.depth, self.get_intrinsics(), self.get_im_poses()):
            pts, _ = geometry.depthmap_to_absolute_camera_coordinates(d.cpu().numpy(),
