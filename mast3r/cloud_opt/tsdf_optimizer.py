...
class TSDFPostProcess:
    ...
    def _backproj_pts3d(self, in_depths=None, in_im_poses=None,
        ...
        dm_to_3d = [geometry.depthmap_to_pts3d(depth[i][None], focal_ex(i), pp=pp[[i]]) for i in range(im_poses.shape[0])]

        def autoprocess(x):
            x = x[0]
            return x.transpose(-2, -1) if len(x.shape) == 4 else x
        return [geometry.geotrf(pose, autoprocess(pt)) for pose, pt in zip(im_poses, dm_to_3d)]
    ...
    def _depth_to_pts3d(self, in_depths=None, in_im_poses=None, in_focals=None, in_pps=None, in_imshapes=None):
        ...
        dm_to_3d = [geometry.depthmap_to_pts3d(depth[i][None], focal_ex(i), pp=pp[i:i + 1]) for i in range(im_poses.shape[0])]

        def autoprocess(x):
            x = x[0]
            H, W, three = x.shape[:3]
            return x.transpose(-2, -1) if len(x.shape) == 4 else x
        return [geometry.geotrf(pp, autoprocess(pt)) for pp, pt in zip(im_poses, dm_to_3d)]
    ...
    def get_dense_pts3d(self, clean_depth=True):
        if clean_depth:
            confs = clean_pointcloud(self.confs, self.optimizer.intrinsics, geometry.inv(self.optimizer.cam2w),
