...
import sys
sys.path.append('/viscam/u/iamisaac/mast3r/dust3r/')
...
class SparseGA():
    def __init__(self, img_paths, pairs_in, res_fine, anchors, canonical_paths=None, cache_path=None):
        def fetch_img(im):
    ...
    def get_dense_pts3d(self, clean_depth=True, subsample=8):
        ...
        if clean_depth:
            confs = clean_pointcloud(confs, self.intrinsics, geometry.inv(self.cam2w), depthmaps, pts3d)
...
def sparse_global_alignment(imgs, pairs_in, cache_path, model, subsample=8, desc_conf='desc_conf',
    ...
    _cache_path = pairs[tuple(imgs)][-1]
    ...
    return SparseGA(imgs, pairs_in, res_fine or res_coarse, anchors, canonical_paths, cache_path=_cache_path)
...
def sparse_scene_optimizer(imgs, subsample, imsizes, pps, base_focals, core_depth, anchors, corres, corres2d,
    ...
    def make_K_cam_depth(log_focals, pps, trans, quats, log_sizes, core_depth):
        ...
        return K, (geometry.inv(cam2w), cam2w), depthmaps
    ...
    def loss_dust3r(cam2w, pts3d, pix_loss):
        ...
        for s in dust3r_slices:
            ...
            tgt_pts = geometry.geotrf(cam2w[s.img2], tgt_pts)
...
def make_pts3d(anchors, K, cam2w, depthmaps, base_focals=None, ret_depth=False):
    ...
    for img, (pixels, idxs, offsets) in anchors.items():
        ...
        pts3d = geometry.geotrf(cam2w[img], pts3d)
...
def extract_correspondences(feats, qonfs, subsample=8, device=None, ptmap_key='pred_desc'):
    ...
    for A, B, QA, QB in [(feat11, feat21, qonf11.cpu(), qonf21.cpu()),
        ...
        # indices of img1 for 1->2 matching and 2->1 matching concatenated
        idx1.append(np.r_[nn1to2[0], nn2to1[1]])
        # indices of img2 for 1->2 matching and 2->1 matching concatenated
...
def backproj(K, depthmap, subsample):
    ...
    xyz = depthmap.unsqueeze(-1) * geometry.geotrf(geometry.inv(K), todevice(uv, K.device), ncol=3)
    return xyz
